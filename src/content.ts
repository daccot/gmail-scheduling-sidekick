(() => {
  ...
  function insertText(text: string) {
    ...
    if (!editable) {
      return {
        ok: false,
        error: "Gmail compose/reply body was not found. Open compose or reply first."
      };
    }
    ...
    if (!ok) {
      editable.innerHTML += (editable.innerHTML ? "<br>" : "") + html;
    }
  ...
  function findEditable(): HTMLElement | null {
    ...
    return nodes.find((n) => {
      ...
      return role === "textbox" ||
        aria.includes("message body") ||
        aria.includes("本文") ||
        n.getAttribute("g_editable") === "true";
